...
int RKEncodeVideo::Initencoder(InputInfo& encoderinfo, int srcindex, PacketCallback  callback, void* userdata)
{
    m_userdata = userdata;
    if(m_is_init) {
        return 0;
...
int RKEncodeVideo::WriteData(const uint8_t *data,int size)
{
    if(!m_is_init) {
        return -1;
    }
...
void RKEncodeVideo::EndEncode()
{
 ...
 {
     if (m_callback) {
        uint32_t pts = m_frame_index * 1000 / m_frame_info.fps;
        m_callback(data, size, pts, m_userdata);
        m_frame_index++;
    }
 }